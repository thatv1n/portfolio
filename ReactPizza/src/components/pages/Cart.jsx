...
import emptyCart from "../../asstets/img/empty-cart.png";
...
function Cart() {
  ...
  const clear = () => {
    if (window.confirm('Вы дейстивтельно хотите очистить корзину?')) dispatch(clearCart())
  }

  const onRemoveItem = (id) => {
    if (window.confirm('Вы дейстивтельно хотите удалить?')) dispatch(removeCartItem(id))
  }
          ...
          !totalCount ?
            ...
            <div className="cart">
              ...
              <div className="content__items">
                {addedPizzas.map((obj, i) => {
                  return (
                    <CartItem id={obj.id} key={i} name={obj.name} size={obj.size} type={obj.type} image={obj.imageUrl} price={obj.price} totalPrice={items[obj.id].totalPrice} totalCount={items[obj.id].items.length}
